...
Lemma Dfn_CVU_implies_Df_exists : forall (fn fn':nat -> R -> R) (f g:R->R)
      ...
      (forall (x:R), lb < x -> x < ub -> Un_cv (fun n => fn n x) (f x)) ->
      (CVU fn' g ((lb + ub)/2) (mkposreal_lb_ub x lb ub lb_lt_x x_lt_ub)) ->
...
intros fn fn' f g x lb ub lb_lt_x x_lt_ub Dfn_eq_fn' fn_CV_f fn'_CVU_g g_cont eps eps_pos.
 ...
 elim (g_cont x lb_lt_x x_lt_ub (eps/8) eps_8_pos) ; clear g_cont ; intros delta1 Hdelta1 ;
 ...
 assert (eps'_pos : 0 < (Rabs h) * eps / 4).
  ...
  fourier.
 elim (fn_CV_f x lb_lt_x x_lt_ub ((Rabs h) * eps / 4) eps'_pos) ; intros N2 fnx_CV_fx.
